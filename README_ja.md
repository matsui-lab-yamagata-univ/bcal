...
# 概要
`bcal` は有機半導体のバンド構造と有効質量を計算するツールです。結晶構造（CIF）を起点に、量子化学計算の入力を生成し、DFT 計算を実行し、分子軌道行列を抽出して強束縛（tight-binding）ハミルトニアンを構築します。得られたバンド分散から HOMO・LUMO のバンド端を求め、それぞれの主有効質量と軸ベクトルを報告し、指定した高対称 k 経路に沿ってバンド図を描画します。
